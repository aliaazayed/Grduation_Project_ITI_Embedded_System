...
/*-------------------------------------------------------------
	      void CLCD_voidDispSpecialChar(u8 * A_u8Array, u8 A_u8Block, u8 A_u8RowId, u8 A_u8ColId)
	      {
...
*/
	      void CLCD_vidSendSpecialChar( u8 * arr_u8Char, u8 cpy_u8BlockNumber)
	      {
	      	CLCD_voidSendCommand(0x40+(cpy_u8BlockNumber*8));  //CGRAM
	      			for(u8 i=0;i<8;i++)
	      			{
	      				CLCD_voidSendData(arr_u8Char[i]);

	      			}
	      	         CLCD_voidSendCommand(0x80);  //DDRAM
	      }
